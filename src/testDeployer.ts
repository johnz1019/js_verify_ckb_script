...
async function DeployUpgradableCell() {
  ...
  const scriptBinPath =
    '/Users/zhang/work/DigiCash/CKB/grands/ckb-system-scripts/specs/cells/ckb_cell_upgrade';
  deployer.deployScript(scriptBinPath, 150000, unspentCells, false);
}

async function DeploySecp256KeccakCell() {
  const unspentCells = await getUnspentCell();
  const scriptBinPath =
    '/Users/zhang/work/DigiCash/CKB/grands/ckb-system-scripts/specs/cells/secp256k1_keccak256_sighash_all';
...
async function DeployKeyBoundOwnershipCell() {
  const unspentCells = await getUnspentCell();
  const scriptBinPath =
    '/Users/zhang/work/DigiCash/CKB/grands/ckb-system-scripts/specs/cells/key_bound_ownership_lock';
...
async function DeploySimpleOtxCell() {
  const unspentCells = await getUnspentCell();
  const scriptBinPath =
    '/Users/zhang/work/DigiCash/CKB/grands/ckb-system-scripts/specs/cells/simple_otx';
  deployer.deployScript(scriptBinPath, 150000, unspentCells, true);
}
