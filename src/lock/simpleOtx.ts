...
export class SimpleOtx {
  ...
  calculateSimpleOtxHash(
    ...
    for (let i = 0; i < len; i++) {
      ...
      hashBytes = hashBytes.concat(
        hexToBytes(serializeInput(tx.inputs[index]))
      );
      ...
      hashBytes = hashBytes.concat(
        hexToBytes(toHexInLittleEndian(output.capacity, 8))
      );
  ...
  signTx(
    ...
    const selectedWitnesses = rawTx.witnesses.slice(
      beginIndex,
      beginIndex + len - 1
    );

    // const selectedWitnesses = rawTx.witnesses;
